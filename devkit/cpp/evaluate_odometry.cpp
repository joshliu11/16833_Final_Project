...
void saveStats (vector<errors> err,string dir, int seq) {
  ...
  string seq_string = to_string(seq);

  // open file  
  FILE *fp = fopen((dir + "/" + seq_string + "_stats.txt").c_str(),"w");
...
bool eval (string result_sha) {
  ...
  for (int32_t i=1; i<7; i++) {
    ...
    if (poses_gt.size()==0 || poses_result.size()!=poses_gt.size()) {
      ...
      printf("Poses results not same size as gt poses!\n");
      continue;
      //return false;
    }
    ...
    if (i<=15) {
      ...
      saveStats(seq_err,result_dir, i);
    }
  }
